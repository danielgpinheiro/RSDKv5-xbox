# ---------------------------------------------------------------------------
# CREDIT: Vendored from the Dreamcast port of RSDKv5 / Sonic Mania — its offline
# asset pipeline (dreamcast/rsdkv5.py). RSDKv5 Data.rsdk archive reader/decryptor,
# used by rsdkv5_extract.py. Unmodified. See tools/README.md.
# ---------------------------------------------------------------------------
import struct
import hashlib
import binascii
...
class RSDKv5File(object):
    ...
    def encrypt_decrypt(self, data, is_decrypt=False):
        if self.key1 is None:
            ...
            key1 = list(swap_hash_endian(hashlib.md5(self.filename.upper().encode()).digest()))
        else:
            key1 = self.key1
    ...
    def guess_unknown_encrypted_key(self):
        ...
        key1_options = [[] for i in range(16)]
        key2 = list(swap_hash_endian(hashlib.md5(str(len(data)).encode()).digest()))
        key1_index = 0
        key2_index = 8
        swap_nibbles = 0
...
class RSDKv5(object):
    def __init__(self, path = None):
        ...
        if path is not None:
            f = open(path, "rb")
            assert f.read(4).decode() == "RSDK", "Invalid signature (expected RSDK)"
            ver = f.read(2).decode()
    ...
    def get_file(self, name):
        hash = hashlib.md5(name.lower().encode()).digest()
    ...
    def get_static_object_path(name):
        hash_bytes = swap_hash_endian(hashlib.md5(name.encode()).digest())
        filename = b"".join([
            binascii.hexlify(hash_bytes[i:i+4])[::-1].upper() for i in range(0, 16, 4)
        ]).decode()
        return f"Data/Objects/Static/{filename}.bin"
